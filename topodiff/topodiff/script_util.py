...
def regressor_defaults():
    return dict(
        image_size=64,
        in_channels=1,         # 추가: 3D 볼륨 입력 채널 수
        out_channels=2,        # 추가: 출력 채널 (VF와 Young's Modulus)
...
def create_model(
    ...
    return UNetModel(
        ...
        in_channels=1,  # 3D 볼륨용으로 수정 (기존: 1+5)
...
def create_regressor_and_diffusion(
    ...
    regressor = create_regressor(
        ...
        regressor_resblock_updown,
        regressor_pool,
        out_channels=out_channels,  # ✅ 여기도 추가
    )
    diffusion = create_gaussian_diffusion(
        steps=diffusion_steps,
        learn_sigma=learn_sigma,
...
def create_regressor(
    ...
    return EncoderUNetModel(
        ...
        out_channels=out_channels,  # 2로 설정됨 (VF, Young's Modulus)
